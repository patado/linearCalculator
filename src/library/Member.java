...
public class Member {
                ...
                if (memberList.get(i).getId()==ID){
                    ...
                    if (status){// if OK to borrow books
                        ...
                        for (int j=0; j<nbreBooksRequested; j++){// instanciates number of loans requested
                            ...
                            if (j==nbreBooksRequested-1){//if this is the last book borrowed, print confirmation on screen
                                in.nextLine();// to flush nextline ()
                                System.out.println("\nThe number of books out for member "+memberList.get(i).getFirstName()+" "+memberList.get(i).getLastName()+" has been updated to "+memberList.get(i).getExistingLoan());
    ...
    public static void freeMemberLoan(int ID){
        for (int i=0; i<memberList.size() ;i++ ){
                if (memberList.get(i).getId()==ID){
                    memberList.get(i).setExistingLoan(-1);
                    break;
                }
        }
    }//end freeMember method
